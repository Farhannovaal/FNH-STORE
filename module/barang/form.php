...
if ($barang_id) {
    ...
    $status = $row['status'];
    $button = "Update";
    $keterangan_gambar="(Klik pilih gambar untuk mengganti gambar disamping)";
...
<form action="<?php echo BASE_URL . 'module/barang/action.php?barang_id='. $barang_id ?>" method="POST" enctype="multipart/form-data">
            ...
            <select name="kategori_id">
                ...
                while ($row = mysqli_fetch_assoc($query)) {
                    if ($kategori_id == $row['kategori_id']) {
                        echo "<option value='{$row['kategori_id']}' selected='selected'>{$row['kategori']}</option>";
                    } else {
    ...
    <div class="element-form">
        ...
        <span><input type="file" name="file"><?php echo$gambar;?>
    </span>
    </div>
    ...
    <div class="element-form">
        ...
        <span>
            ...
            <input type="radio" name="status" value="off" <?php if($status == "off"){ echo "checked='checked'";} ?> />Off
        </span>
    </div>
